...
import { RetrieveTopCommentsFromCommentRequest } from '../proto-auto-gen/reddit/RetrieveTopCommentsFromCommentRequest.ts';
...
export const RetrieveTopComments: RedditServiceHandlers['RetrieveTopComments'] = async (
    call: ServerUnaryCall<RetrieveTopCommentsFromCommentRequest, RetrieveTopCommentsResponse>,
    ...
    try {
      ...
      const topComments = await new Promise<Comment[]>((resolve, reject) => {
        db.all(topCommentsQuery, [request.commentId, request.n], (err, rows: Comment[]) => {
          if (err) {
            reject(err);
            return;
          }
